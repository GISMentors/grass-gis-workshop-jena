Unit 18 - Lidar, DSM interpolation
==================================
...
For importing LAS/LAZ data there are available two GRASS tools:
...
.. note:: GRASS requires `libLAS <https://liblas.org/>`__ library for
   ...
   There is also a new generation of Lidar tools based on `PDAL
   <https://pdal.io/>`__ library: :grasscmd:`r.in.pdal` and
   :grasscmd:`v.in.pdal`. 

.. important:: Unfortunately on MS Windows PDAL-based tools are
   missing due to compilation problems (`#1821
   <https://github.com/OSGeo/grass/issues/1821>`__) and libLAS-based
   tools lack compression support.
...
Let's create a new GRASS project *jena-lidar* defined by
:epsg:`25832` (the SRS in which the Lidar data is referenced).

.. figure:: ../images/units/18/create-project-epsg.png

   Create a new GRASS project by specifying EPSG code (*Select CRS
...
There are several GRASS tools for filling gaps in raster maps like
:grasscmd:`r.fillnulls` or :grasscmd:`r.fill.stats`. The first tool
is based on spline interpolation, the second fills gaps with
interpolated values using IDW. Let's use the second tool which fill
...
.. note:: Don't forget to adjust computation region before running
   :grasscmd:`r.fillnulls`.

   .. code-block:: bash

      g.region raster=las_681_5644
...
is built, the tool must scan all points in the map):
