Unit 04 - Tools, Region
=========================

.. _grass-tools:

Accessing GRASS tools
-----------------------

GRASS is a **modular system** which consists of several hundreds
*tools* ("tools" in GRASS terminology). Tools are accessible from
...
.. figure:: ../images/units/04/tools-tab.png

   Search for a tool by 'clip' keyword in :item:`Tools` tab. The
   tool which we are looking for is :grasscmd:`v.clip`. Tool
   dialog can be open by :item:`Run` button.

.. note:: Tool :grasscmd:`v.clip` was introduced to GRASS in version
   7.4.0. In older versions generic :grasscmd:`v.overlay` tool can
   be used.
   
.. figure:: ../images/units/04/tools-cmd.png

   Launching :grasscmd:`v.clip` tool from :item:`Console` tab.

Tools (formerly "Modules") can be run from GUI dialogs, from :item:`Console` tab,
or by using Python API (see :doc:`10`). Figure bellow shows GUI dialog
of :grasscmd:`v.clip` tool.

.. _v-clip:

.. figure:: ../images/units/04/v-clip.png

   GUI dialog for launching :grasscmd:`v.clip` tool.

.. tip:: CLI syntax of tools is shown in GUI dialog statusbar, see
...
Each GRASS tool starts by a prefix. This prefix groups tools into
...
The great majority of raster processing tools (``r.*``) respect the 
computational region. There are a few exceptions like import tools
(eg. :grasscmd:`r.import`). On the other hand, most vector
processing tools (``v.*``) ignore the computational region completely
...
.. tip:: Full flexibility for operating with computational region is allowed
   via the :grasscmd:`g.region` tool (:menuselection:`Settings -->
   Computational region --> Set region`). Example below:
