...
Let's import :file:`osm/counties.gpkg` into *jena-region*
GRASS project as described in :doc:`03`. In this case coordinate
...
project (``Projection match: No``). This operation needs to be
confirmed by a user, see :numref:`confirm-reprojection`.

.. _confirm-reprojection:

.. figure:: ../images/units/14/reproject.png
   :class: small
           
   Confirm reprojection of input data.

Data incosistency can cause significant topological errors when
importing vector data into GRASS (as in our case
:numref:`topo-errors`):

::

   WARNING: Number of centroids exceeds number of areas: 47 > 46
   WARNING: Number of incorrect boundaries: 11
   WARNING: Number of centroids outside area: 2

.. _topo-errors:

.. figure:: ../images/units/14/counties-broken.png
   :class: middle
           
   Topological errors detected by import process.

Usually helps to set up a "reasonable" snapping threshold when
importing data into GRASS, see :numref:`import-snap`.

.. _import-snap:

.. figure:: ../images/units/14/threshold_snap.png

   Set threshold for snapping before data import in :item:`Import
   settings` tab.
...
.. code-block:: python
   ...
   for feat in counties.viter('areas'):
       if not feat.attrs:
           continue
       print ('{0}: {1:.1f}ha'.format(feat.attrs['name'], feat.area() / 1e4))
...
::
   ...
   Hamburg: 314.8ha
   Hamburg: 265.6ha
   Niedersachsen: 3063.6ha
   Niedersachsen: 2792.4ha
   Niedersachsen: 2056.9ha   
   ...
