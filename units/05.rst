...
index). The key GRASS tool that allows a user to do the map algebra
...
.. figure:: ../images/units/05/ndvi-mapcalc.png
   ...
   NDVI computed by :grasscmd:`r.mapcalc` map algebra tool (Jena
   city in red color).
...
can be created by :grasscmd:`r.mask`. Like other tools from the
...
.. important:: **The mask is used globally in the given mapset for each
   subsequent calculation operation until it is deleted.**
...
Note that there is specialized tool for computing various vegetation
indices including NDVI - :grasscmd:`i.vi`. This tool makes NDVI
