...
.. _create-project-epsg:

Create a new GRASS project *germany-modis* (see :ref:`Unit 02
<create-project>`) using EPSG code (*Select CRS from a list by EPSG
or description*).

.. figure:: ../images/units/24/create-project-epsg-0.png

   Create a new project based on EPSG code.

.. figure:: ../images/units/24/create-project-epsg-1.png
...
GRASS MODIS addon consists of two tools:
...
Input MODIS data can be imported and reprojected into target project
by :grasscmdaddons:`i.modis.import`.

.. code-block:: bash

   i.modis.import -qmw files=modis/listfileMOD11A2.061.txt outfile=tlist-mod.txt

   i.modis.import -qmw files=modis/listfileMYD11A2.061.txt outfile=tlist-myd.txt
...
:file:`osm/counties.gpkg` into the current GRASS project as described
in :doc:`03`.
...
Let's check range values of imported MODIS data by :grasscmd:`r.info` tool:
