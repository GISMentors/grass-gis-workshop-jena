...
:pygrass-tools:`ParallelToolQueue` similarly as in :doc:`19`, see
line :lcode:`209`.

New PyGRASS functionality is introduced on line :lcode:`132`. By
:pygrass-tools:`MultiTool` it is possible to define a list of
tools which will work as isolated units not influenced by other
...
``sync=True`` all tools will run in serial order in a background
...
parallel computation (the tool creates raster map named
