...
`sentinel_ndvi` (don't use dashes in the mapset name otherwise
:grasscmd:`t.rast.algebra` will most probably fail).

.. todo:: Fix :grasscmd:`t.rast.algebra` to support also mapsets
          containing dash(es) in the name.

Download data
-------------

.. important:: Pre-downloaded Sentinel scenes are available in the
   sample dataset :file:`geodata/sentinel`. Readerscan continue
...
.. todo:: Generate raster mask by :grasscmdaddons:`i.sentinel.import` (?)
...
be created with :grasscmd:`r.mask`, see line :lcode:`50`. But in this
...
:grasscmd:`g.rename`, see line :lcode:`52`.
