...
Flooding can be easily simulated by :grasscmd:`r.lake` tool which
...
.. note:: Before running :grasscmd:`r.lake` let's start **organizing
   ...
   *flooding* in the current project *jena-region*. New mapset from
   :item:`Data` tab or by :grasscmd:`g.mapset` tool.
