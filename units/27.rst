...
In this unit we will demonstrate how to write your own WPS
processes. Our processes will be tested in simple environment of
`PyWPS demo <https://github.com/geopython/pywps-flask>`__. `Download
<https://github.com/geopython/pywps-flask/archive/master.zip>`__ the
demo and extract into your working directory.

Then install PyWPS and Flask dependency

.. code-block:: bash

   sudo pip install pywps flask

Go to :file:`pywps-flask` directory and try to start your PyWPS demo
server
...
Then go to http://127.0.0.1:5000/ in order to see all available demo
processes. Let's try to show process description for selected process,
eg. *say_hello* process. Try to run this process.
...
:file:`processes` (you can take :file:`grassbuffer.py` as
template). Example of process below is based on script created in
...
:lcode:`13`). Every process has its identifier (line :lcode:`18`),
title and abstract. The process will operate in GRASS location, it is
defined on line :lcode:`28`. PyWPS creates in this location for each
job (executed process by client) temporary mapset which is deleted
when process is finished. Process body is always implemented as
``_handler()`` method, see line :lcode:`38`. Resultant statistics is
stored to response output as simple string on line :lcode:`78`.

.. literalinclude:: ../_static/scripts/modis_v1.py
   :language: python
   :linenos:
   :emphasize-lines: 7, 9, 11, 13, 18, 28, 38, 78
                     
Then we have to activate the process in :file:`demo.py` (which
implements PyWPS demo server). Assuming that you named file with
process as :file:`modis_v1.py`.
...
.. important:: In order to use GRASS GIS functionality check PyWPS
   configuration. Go to :file:`pywps.cfg` and set correct GRASS
...
.. note:: Try to rewrite process in order to return something more
          usable than string, eg. :wikipedia-en:`JSON`.
