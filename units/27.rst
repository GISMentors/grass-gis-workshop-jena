...
In GRASS terminal go to :file:`pywps-flask-master` directory (by
:program:`cd` command) and start PyWPS demo server:
...
1. The option :param:`where` will be split into two options
   (:lcode:`17-20,55-57`): :param:`start` - Start date and
   :param:`end` - End date.

2. New :param:`output` parameter is defined (:lcode:`22-23`).

3. New function ``check_date()`` to validate input dates will be added
   (:lcode:`4,404-43,45-45`).

4. Output of :grasscmd:`r.univar` will be parsed and statistics
   printed (:lcode:`3,68,70-74`).
...
.. literalinclude:: ../_static/scripts/modis_v1.py
   :language: python
   :linenos:
   :emphasize-lines: 3,4,11,25,17-20,22-23,27,40-43,45-46,55-57,68,70-74,82

Source code comments: process itself is defined as a Python class,
:class:`ModisV1` in this case (line :lcode:`11`). In class constructor
input (lines :lcode:`17-20`) and output parameters (line
:lcode:`22-23`) are defined. Every process has its identifier (line
:lcode:`27`), title and abstract. The process will operate in GRASS
location defined on line :lcode:`32`. On line :lcode:`50` is assumed
...
line :lcode:`39`. Resultant statistics is stored to response output as
a simple string on line :lcode:`74`.
