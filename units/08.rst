...
Modeler, a chain of tools (ie. GRASS tools) can be wrapped
...
:menuselection:`Modeler --> Model --> Add command` or by |grass-module-add|
:sup:`Add GRASS tool (module) to model` from the main toolbar.

.. figure:: ../images/units/08/add-tool.png

   Add GRASS tool (tool) to model, in this case :grasscmd:`r.mask`.

.. important:: Graphical Modeler sometimes has a problem with rendering
   the model. In this case it helps to redraw model canvas by
   |grass-redraw| :sup:`Redraw model canvas`.

After adding :grasscmd:`r.mask` tool to the model let's open its
...
.. figure:: ../images/units/08/tool-props.png
...
data produced by model (:menuselection:`Modeler --> Model --> Model
