...
Let's step out of the GRASS environment for a moment. `QGIS
...
third-party software packages as SAGA, Orfeo or GRASS. GRASS tools
can be run directly from QGIS enviroment without starting GRASS
explicitly.

Data produced by GRASS may be easily accessed directly from
:item:`Browser` tab.

.. figure:: ../images/units/07/qgis-display-grass-data.png
   :class: middle
           
   Show data produced by GRASS in QGIS environment.
...
input data: Jena city region (:file:`osm/jena_boundary.gpkg`)
and Sentinel bands (red and near-infrared channels) from
:file:`sentinel/sample` directory.
...
:grasscmd:`i.vi` GRASS tool.

.. figure:: ../images/units/07/processing-i-vi.png
   :class: small
           
   Search for :grasscmd:`i.vi` GRASS tool in Processing toolbox.
...
works. First of all a temporary GRASS project is created by
:grasscmd:`g.proj`, input data is linked into temporary project by
:grasscmd:`r.external`. Then computation can finally starts by running
:grasscmd:`i.vi` tool. In the last step output data is exported out
